...
- [ ] No match replay functionality (viewer / sharing)
    - **Done (persistence):** On each successful move, `GameInterface` calls `Database.record_move` and
      ...
      `peek` = false). Bot moves recorded with bot `user_id`. `Game.attach_replay_logger` / `Game.log_replay_event` exist
...
- [x] Better match history summaries (foundation)
    ...
    - **Still TODO:** Implement `match_summary` for other games (chess, poker, etc.) with rich text like “checkmate in N.”
...
- [ ] Add a better way to limit the bot's scope (full product vision)
    - **Partial today:** Guild setting `playcord_channel_id` in `guilds.settings`; `/playcord set_channel` (admin);
      onboarding mentions it; [`playcord.require_playcord_channel`](configuration/config.yaml) forces `/play` to run only
...
- [ ] Analytics viewer of some kind
    - **Partial:** `/playcord analytics` (bot owners) — event counts by type from DB for a look-back window. No charts UI
      yet.
...
- [x] Pagination after bot restarts (UX)
    - **Done:** Pagination callbacks `defer()` immediately; global `interaction` listener in [`bot.py`](bot.py) replies
      with [`interactions.pagination_outdated`](configuration/locale/en.toml) when no view handled the component.
