    ...
    if owner is not None:
        ...
        if oid is not None:
            try:
                oid_i = int(oid)
                ids.add(oid_i)
                log.debug("portal_owner_ids_from_appinfo: found application owner id=%d", oid_i)
            except (TypeError, ValueError):
                log.warning("portal_owner_ids_from_appinfo: invalid owner id value=%r", oid)
    ...
    if team is not None:
        ...
        if toid is not None:
            try:
                toid_i = int(toid)
                ids.add(toid_i)
                log.debug("portal_owner_ids_from_appinfo: found team owner id=%d", toid_i)
            except (TypeError, ValueError):
                log.warning("portal_owner_ids_from_appinfo: invalid team owner id value=%r", toid)
...
async def resolve_effective_owner_ids(client: discord.Client) -> frozenset[int]:
    ...
    log.debug("resolve_effective_owner_ids: starting with STATIC_OWNER_IDS=%s", sorted(merged))

    try:
        log.debug("resolve_effective_owner_ids: requesting application_info() from client=%r", client)
    ...
    try:
        ...
        log.debug("resolve_effective_owner_ids: portal owner ids=%s", sorted(portal_ids))
    except Exception:
        log.exception("resolve_effective_owner_ids: failed to extract portal owner ids from app_info=%r", app_info)
