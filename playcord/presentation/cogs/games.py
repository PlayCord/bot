...
async def handle_move(
    ...
    # Both public and private threads are valid, error without an additional check if it's not one of those
    if ctx.channel.type not in [
        ...
        await followup_send(
            ctx,
            content=(get("move.invalid_context")),
            ephemeral=True,
            delete_after=EPHEMERAL_DELETE_AFTER,
        )
        return
    ...
    if requested_game_type and requested_game_type != active_game.game_type:
        ...
        await followup_send(
            ctx,
            content=(
                f"{get('move.invalid_context')}. "
                f"{fmt('move.wrong_game_type', game=active_game.game_type)}"
            ),
            ephemeral=True,
            delete_after=EPHEMERAL_DELETE_AFTER,
        )
