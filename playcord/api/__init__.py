"""Final runtime game API used by GameManager."""
...
class RuntimeGame(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        # Wired by ``GameManager`` for ``log_replay_event`` → ``replay_events``.
    ...
    def log_replay_event(self, event_type: str, **payload: Any) -> None:
        """Append a replay row when ``_replay_hook`` is set (by ``GameManager``)."""
