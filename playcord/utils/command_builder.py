...
def encode_argument(argument_name, argument_information) -> str:
    if argument_information["type"].__class__ is RangeTransformer:
        ...
        if option_type == AppCommandOptionType.integer:
            ...
            range_type = str
        min_value, max_value = (
            argument_information["type"]._min,
            argument_information["type"]._max,
        )
        argument_type = (
            f"app_commands.Range[{range_type.__name__}, {min_value}, {max_value}]"
        )
    else:
...
def encode_decorator(decorator_type, decorator_values) -> str:
    ...
    for command_argument in decorator_values:
        stringified_arguments.append(
            f"{command_argument}={str(decorator_values[command_argument])}"
        )
    ...
    for game in GAME_TYPES:
        ...
        for move in moves:
            ...
            for option in move.options:
                ...
                if (
                    "autocomplete" not in option_decorators
                    and option.autocomplete is not None
                ):
                    option_decorators.update(
                        {
                            "autocomplete": {
                ...
                for decorator in option_decorators:
                    if decorator not in decorators:
                        ...
                        temp_decorators[decorator].update(
                            {decorator: option_decorators[decorator]}
                        )
            decorators[move.name] = temp_decorators
            arguments[move.name] = temp_arguments
        ...
        for this_move in moves:
            ...
            encoded_decorators = [
                encode_decorator(d, this_move_decorators[d])
                for d in this_move_decorators
            ]
            encoded_arguments = [
                encode_argument(a, this_move_arguments[a]) for a in this_move_arguments
            ]
            ...
            if "autocomplete" in this_move_decorators:
                for autocomplete in this_move_decorators["autocomplete"]:
                    ...
                    ac_command = (
                        f"async def {ac_command_name}(ctx, current):\n"
