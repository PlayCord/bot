...
from playcord.utils.conversion import (
    column_creator,
    column_elo,
    column_names,
    contextify,
...
class MatchmakingInterface:
    ...
    def __init__(
        ...
        self.whitelist = {
            get_container().players.get_discord_player(creator, message.guild.id)
        }
    ...
    async def seed_rematch_players(
        ...
        for uid in user_ids:
            ...
            player = get_container().players.get_discord_player(member, guild.id)
            if player is None:
    ...
    async def ban(self, player: discord.User, reason: str) -> str | None:
        ...
        new_player = get_container().players.get_discord_player(
            player, self.message.guild.id
        )
        ...
        kicked = False
        if MatchmakingInterface._remove_queued_player(self, new_player.id) is not None:
            kicked = True
