...
from playcord.discord_games.game import PlayerOrder
from playcord.discord_games.message_components import Container, Message, TextDisplay
from playcord.discord_games.player import Player
from playcord.discord_games.response import Response
...
from playcord.utils.analytics import Timer, register_event
from playcord.utils.containers import (
    LoadingContainer,
    container_send_kwargs,
    container_to_markdown,
)
from playcord.utils.conversion import contextify, textify
from playcord.utils.database import InternalPlayer, internal_player_to_player
from playcord.utils.discord_utils import followup_send
from playcord.utils.interface_lifecycle import game_over
from playcord.utils.interface_rendering import build_game_info_message, build_game_status_view
from playcord.utils.interfaces import synthetic_bot_name_from_id
...
from playcord.utils.error_reporting import try_send_error_embed
        ...
        try:
            ...
            async def edit_info_message():
                try:
                    ...
                    log.exception("edit_info_message failed")
                    self.ending_game = True
                    try:
                        await try_send_error_embed(
                            log,
                            _e,
                        ...
                        log.exception("try_send_error_embed failed in edit_info_message")
                    # Schedule ending the game from a separate task to avoid deadlock with pending UI tasks
                    try:
                        asyncio.create_task(game_over(self, str(_e)))
                    except Exception:
                        log.exception("scheduling game_over failed in edit_info_message")
            ...
            async def edit_game_message():
                try:
                    ...
                    log.exception("edit_game_message failed")
                    self.ending_game = True
                    try:
                        await try_send_error_embed(
                            log,
                        ...
                        log.exception("try_send_error_embed failed in edit_game_message")
                    # Schedule ending the game from a separate task to avoid deadlock with pending UI tasks
                    try:
                        asyncio.create_task(game_over(self, str(_e)))
                    except Exception:
                        log.exception("scheduling game_over failed in edit_game_message")
            ...
            async def edit_status_message():
                try:
                    ...
                    log.exception("edit_status_message failed")
                    self.ending_game = True
                    try:
                        await try_send_error_embed(
                            log,
                        ...
                        log.exception("try_send_error_embed failed in edit_status_message")
                    # Schedule ending the game from a separate task to avoid deadlock with pending UI tasks
                    try:
                        asyncio.create_task(game_over(self, str(_e)))
                    except Exception:
                        log.exception("scheduling game_over failed in edit_status_message")
            ...
            # Synchronous failure while preparing UI tasks — report and mark ending
            ...
            try:
                ...
                log.exception("try_send_error_embed failed in display_game_state except block")
            # End the game for synchronous failures (safe to await here)
            try:
                await game_over(self, str(e))
            except Exception:
                log.exception("game_over failed in display_game_state except block")
