...
def internal_player_to_player(
    ...
    if user is not None:
        ...
        uname = (
            ...
            else (internal_player.name or f"User {uid}")
        )
    else:
        uid = internal_player.id
        uname = internal_player.name or (
            f"User {uid}" if uid is not None else "Unknown"
        )
    return Player(
        mu=rating.mu,
        sigma=rating.sigma,
...
class Database:
    ...
    def _load_schema_asset(self) -> None:
        ...
        Thin wrapper around :meth:`_load_sql_asset` for ``schema.sql`` so
        tests can mock this entry point without affecting other SQL assets.
        """
        self._load_sql_asset("schema.sql")
    ...
    def append_replay_event(self, match_id: int, event: dict[str, Any]) -> None:
        """
        Append one structured replay event to replay_events (canonical store).
        ``type`` is stored in replay_events.event_type and ``user_id`` maps to actor_user_id.
        """
    ...
    def get_replay_events(self, match_id: int) -> list[dict[str, Any]]:
        """
        Return replay events in order.

        Canonical source is replay_events.
        """
        ...
        for row in rows:
            ...
            event_type = str(row.get("event_type") or payload.pop("event_type", "event"))
    ...
    def get_player(
        ...
        user_id = (
            user.id
            if isinstance(user, (discord.User, discord.Member, InternalPlayer))
            else user
        )
